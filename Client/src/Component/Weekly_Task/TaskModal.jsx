...
const TaskModal = ({ closeTaskModal, taskData }) => {
  ...
  const [values, setValues] = useState({
    ...
    manager_id: taskData ? taskData.manager_id : "",
  ...
  const fetchEmployee = () => {
    axios
      ...
      .catch((err) => {
        toast.error("You are not authorized! employee🔒");
        navigate("/login");
        return;
      });
  };
  ...
  useEffect(() => {
    if (taskData) {
      setValues({
        ...
        manager_id: taskData.manager_id,
    ...
    try {
      ...
      if (taskData) {
        ...
        response = await axios.post(
          "http://localhost:8081/weekly-task/task-assigne/create/task",
          values,
          {
            headers: { "Content-Type": "application/json" },
          }
      ...
      <div className="modal-container">
        ...
        <form className="emp_form" onSubmit={handleSubmit}>
          ...
          <div className="field_t">
            ...
            <div className="col-75">
              <select
                value={values.tag_user_id}
                onChange={(e) =>
                  setValues({ ...values, tag_user_id: e.target.value })
                }
              >
